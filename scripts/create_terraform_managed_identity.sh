...
# Expected argument format: org/repository.
GITHUB_ORG_REPO="${1:-}"
...
# Validate required GitHub repository argument.
...
# Authenticate to Azure and select target subscription.
az login --tenant "$TENANT_ID" --allow-no-subscriptions >/dev/null
az account set --subscription "$SUBSCRIPTION_ID"

# Create (or update) resource group for the managed identity.
az group create \
  --name "$IDENTITY_RG" \
  --location "$IDENTITY_LOCATION" \
  --output none

# Create (or update) the user-assigned managed identity.
az identity create \
  --name "$IDENTITY_NAME" \
...
# Capture identity IDs for later configuration output.
...
# Grant subscription-level Contributor role (idempotent).
...
# Create GitHub OIDC federated credential for main branch (idempotent).
...
# Print identity details and values to set in GitHub.
